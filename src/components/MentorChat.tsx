...
export function MentorChat({ isWidget = false }: { isWidget?: boolean }) {
  ...
  const handleSend = async (textToUse?: string) => {
    ...
    if (!textToUse) {
      ...
      // Re-add the user message just to be sure it's at the bottom
      if (!messages.find((m) => m.content === userText && m.role === 'user')) {
        setMessages((prev) => [
          ...prev,
          { id: Date.now().toString(), role: 'user', content: userText },
        ])
      ...
      >
            ...
            {messages.map((msg) => (
              ...
              >
                ...
                >
                  <div
                    className={cn(
                      'p-3 md:p-4 rounded-2xl text-sm md:text-base leading-relaxed break-words',
                      msg.role === 'ai'
                        ? msg.isError
        ...
        <div
          className={cn(
            ...
            isWidget && 'pb-6', // extra padding for safe area on mobile widget
          )}
        >
          <form
            onSubmit={(e) => {
